...
&Тест
Процедура ОтправитьСообщениеСотрудникуКомпанииВСоответствие() Экспорт

	Текст = "Здравствуйте. Это тестовое сообщение";
	Метод = ПолучитьБота().ОтправитьСообщениеСотрудникуКомпании()
				.ИдПолучателя("cd484afc-29b7-11ed-8dca-00505601495b")
				.ИдАвтора("dd9f0a89-08db-49bf-83dc-dfe3269bf6fe")
				.Текст(Текст);
	Карта = Метод.ВСоответствие();
	Ожидаем.Что(Метод.recepient_id).Равно("cd484afc-29b7-11ed-8dca-00505601495b")
		   .Что(Метод.author_id).Равно("dd9f0a89-08db-49bf-83dc-dfe3269bf6fe")
		   .Что(Метод.text).Равно(Текст)
		   .Что(Карта["recepient_id"]).Равно("cd484afc-29b7-11ed-8dca-00505601495b")
		   .Что(Карта["author_id"]).Равно("dd9f0a89-08db-49bf-83dc-dfe3269bf6fe")
		   .Что(Карта["text"]).Равно(Текст)
	;
...
&Тест
Процедура ОтправитьСообщениеСотрудникуКомпанииВызвать() Экспорт

	Текст = "Здравствуйте. Это тестовое сообщение сотруднику компании";
	Метод = ПолучитьБота().ОтправитьСообщениеСотрудникуКомпании()
				.ИдПолучателя("cd484afc-29b7-11ed-8dca-00505601495b")
				.ИдАвтора("dd9f0a89-08db-49bf-83dc-dfe3269bf6fe")
				.Текст(Текст)
...
&Тест
Процедура ОтправитьСообщениеВГруппуВСоответствие() Экспорт

	Текст = "Здравствуйте. Это тестовое сообщение";
	Метод = ПолучитьБота().ОтправитьСообщениеВГруппу()
				.ИдГруппы("cd484afc-29b7-11ed-8dca-00505601495b")
				.ИдАвтора("dd9f0a89-08db-49bf-83dc-dfe3269bf6fe")
				.Текст(Текст);
	Карта = Метод.ВСоответствие();
	Ожидаем.Что(Метод.conference_id).Равно("cd484afc-29b7-11ed-8dca-00505601495b")
		   .Что(Метод.author_id).Равно("dd9f0a89-08db-49bf-83dc-dfe3269bf6fe")
		   .Что(Метод.text).Равно(Текст)
		   .Что(Карта["conference_id"]).Равно("cd484afc-29b7-11ed-8dca-00505601495b")
		   .Что(Карта["author_id"]).Равно("dd9f0a89-08db-49bf-83dc-dfe3269bf6fe")
		   .Что(Карта["text"]).Равно(Текст)
	;
...
&Тест
Процедура ОтправитьСообщениеВГруппуВызвать() Экспорт

	Текст = "Здравствуйте. Это тестовое сообщение в группу";
	Метод = ПолучитьБота().ОтправитьСообщениеВГруппу()
				.ИдГруппы("cd484afc-29b7-11ed-8dca-00505601495b")
				.ИдАвтора("dd9f0a89-08db-49bf-83dc-dfe3269bf6fe")
				.Текст(Текст)
